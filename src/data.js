...
window.KINTORE_DATA = {
  ...
  meta: {
    ...
    weekNow: 7,        // ★今が何週目か（要確認・トレ報告ごとに更新）
    weekTotal: 12,     // 12週目はディロード（軽め）週
    deloadWeek: 12,
    phaseNote: "Phase 2：主要種目の重量を上げていく時期。Week 12 はディロード（全重量60〜70%・2セット）で成長を確定させる。",
    overview: "上半身（Upper）と下半身（Lower）をそれぞれA・Bの2種類で回す4分割。各部位を週1.5回ずつ刺激して、筋肥大に必要な頻度を確保する。重い主役種目＋それを支える補助種目という構成で、毎回どこかを「伸ばし」どこかを「維持」する。",
  ...
  progress: [
    ...
    { lift: "ショルダープレス", start: 41, current: 54, unit: "kg" }
  ],

  /* これまでのトレーニング記録（新しい順）。トレ報告のたびに先頭へ1行追加する。 */
  log: [
    { date: "2026-06-22", sessionKey: "upperB", minutes: 85, highlights: "ショルダー54×12・10・7／ペック54×12・12・11／EZカール30×12・6" }
  ]
};
